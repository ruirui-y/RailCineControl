...
public slots:
    void SlotPostSqlQueryTask(const QString& sql, QueryCallback cb, bool bIsAsync, const QList<QVariant>& params = QList<QVariant>());
    void SlotPostSqlUpdateTask(const QString& sql, UpdateCallback cb, bool bIsAsync, const QList<QVariant>& params = QList<QVariant>());
    void SlotPostSqlTransactionTask(const QList<QString>& sqls, TransactionCallback cb, bool bIsAsync, const QVariantList& allParams = QList<QVariant>());

private:
    std::atomic<QObject*> dispatcher_{ nullptr };
    QSemaphore ready_{ 0 };
    QSharedPointer<SqlExec> sqlExec;
};
